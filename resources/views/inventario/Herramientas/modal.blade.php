<div class="modal fade model-side-in-right" role="dialog" tabindex="-1" aria-hidden="true" id="modal-delete-{{$tras->id}}">
	<form action="{{ route('Herramientas.destroy',['Herramienta' => $tras->id]) }}" method="post">
		@method("DELETE")
		@CSRF
		<div class="modal-dialog"  role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Baja de Herramienta {{$tras->id}}</h4>
				</div>
				<div class="modal-body">
					confirme si desea dar de baja la herramienta
...
<div class="modal fade model-side-in-right" role="dialog" tabindex="-1" aria-hidden="true" id="modal-activate-{{$tras->id}}">
	<form action="{{ route('Herramientas.activar',['Herramientas' => $tras->id]) }}" method="post">
		@CSRF
		<div class="modal-dialog"  role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Activar Herramienta {{$tras->id}}</h4>
				</div>
				<div class="modal-body">
					confirme si desea activar la herramienta
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
					<button type="submit" class="btn btn-primary" >Confirmar</button>
