			...
			<table class="table table-striped table-bordered table-condesed table-hover">
				...
				@foreach ($EquipoTransporte as $tras)
					@if(auth()->user()->role_id<3 || auth()->user()->id == $tras->created_user)
						<tr>
							<td>{{$tras->id_equipo_transporte}}</td>	
							...
							<td>
								@php
									$serversUser = \App\Models\Imagen::where('id_equipo', '=', $tras->id_equipo_transporte)->where('id_area', '=', 1)->take(1)->get();
								@endphp
							...
							<td>
								<a href="{{route('EquipoTransporte.show', ['EquipoTransporte' => $tras->id_equipo_transporte])}}"><button class="btn btn-info">Ver</button></a>
								<a href="{{route('EquipoTransporte.edit',['EquipoTransporte' => $tras->id_equipo_transporte])}}"><button class="btn btn-info">Editar</button></a>
							
								@if($tras->estado==1)
									<a href="" data-target="#modal-delete-{{$tras->id_equipo_transporte}}" data-toggle="modal"><button class="btn btn-danger">Baja</button></a>
								@else
									<a href="" data-target="#modal-activate-{{$tras->id_equipo_transporte}}" data-toggle="modal"><button class="btn btn-success">Activar</button></a>
								@endif
